...
# ("Regiones", "Internet", "Menciones" o "Mapa_Temas") > formato CSV > copias
# la URL resultante y la guardas en .streamlit/secrets.toml (o en la config de
...
# INTERNET_CSV_URL   = "https://docs.google.com/spreadsheets/d/e/2PACX-xxxx/pub?gid=111&single=true&output=csv"
# MENCIONES_CSV_URL  = "https://docs.google.com/spreadsheets/d/e/2PACX-xxxx/pub?gid=222&single=true&output=csv"
# MAPA_TEMAS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-xxxx/pub?gid=333&single=true&output=csv"
#
# Cada hoja debe tener 2 columnas: la primera con el valor original (medio,
# mención, etc.) y la segunda con el valor mapeado (región, medio normalizado,
# mención mapeada o tema final). Se mantiene el mismo orden de columnas que
# usaba el archivo Configuracion.xlsx anterior, con encabezados en la
# primera fila de cada hoja.
REQUIRED_CSV_SECRETS = [
    "REGIONES_CSV_URL",
    "INTERNET_CSV_URL",
    "MENCIONES_CSV_URL",
    "MAPA_TEMAS_CSV_URL",
...
def load_config_from_sheets():
    """
    Carga region_map, internet_map, mention_map y final_topic_map desde hojas
    de Google Sheets publicadas como CSV (en vez de Configuracion.xlsx).
    Lanza RuntimeError si faltan URLs en st.secrets o si alguna hoja no puede
    leerse, para que el llamador pueda mostrar un st.error() adecuado.
    """
    missing = [k for k in REQUIRED_CSV_SECRETS if k not in st.secrets]
    if missing:
        raise RuntimeError(
            "Faltan estas claves en Secrets (.streamlit/secrets.toml): "
            + ", ".join(missing)
        )

    try:
        df_regiones = pd.read_csv(st.secrets["REGIONES_CSV_URL"])
        df_internet = pd.read_csv(st.secrets["INTERNET_CSV_URL"])
        df_menciones = pd.read_csv(st.secrets["MENCIONES_CSV_URL"])
        df_mapa_temas = pd.read_csv(st.secrets["MAPA_TEMAS_CSV_URL"])
    except Exception as e:
        raise RuntimeError(f"No se pudo leer alguna hoja de Google Sheets: {e}")
    ...
    # Igual que antes: claves normalizadas de forma estricta para evitar
    # fallos por espacios/guiones distintos.
    mention_map = {}
...
<div class="card">
    ...
    <div class="step">
        ...
        <div class="step-text">Asegúrate de que <strong>pipeline_sentimiento.pkl</strong> y
        <strong>pipeline_tema.pkl</strong> estén en la raíz del repositorio, y que las 4 URLs de
        Google Sheets (Regiones, Internet, Menciones, Mapa_Temas) estén configuradas en
        <strong>Secrets</strong>.</div>
    </div>
    <div class="step">
...
with st.expander("📋  Ver estructura requerida para la configuración en Google Sheets"):
    st.markdown("""
    La configuración ya **no** se lee desde `Configuracion.xlsx`, sino desde 4 hojas de
    Google Sheets publicadas como CSV. Cada URL se guarda en Secrets con estas claves:

    | Clave en Secrets | Hoja equivalente | Columna A | Columna B |
    |---|---|---|---|
    | `REGIONES_CSV_URL` | `Regiones` | Medio | Región |
    | `INTERNET_CSV_URL` | `Internet` | Medio Original | Medio Mapeado |
    | `MENCIONES_CSV_URL` | `Menciones` | Mención Original | Mención Mapeada |
    | `MAPA_TEMAS_CSV_URL` | `Mapa_Temas` | Temas Generales - Tema | Tema |

    Para publicar cada hoja: **Archivo > Compartir > Publicar en la web**, eliges la hoja
    específica, formato **CSV**, y copias la URL resultante (tipo
    `.../pub?gid=XXXX&single=true&output=csv`) en `.streamlit/secrets.toml` o en la sección
...
if start_clicked and files_loaded:
    ...
    for f, label, state_key in files_loaded:
        ...
        # 1. Carga de recursos
        progress_bar.progress(5, text="Paso 1 / 7 — Cargando modelos PKL y configuración de Google Sheets...")
        ...
        try:
            ...
            st.error(f"**Error al cargar la configuración desde Google Sheets:** {e}")
            continue
